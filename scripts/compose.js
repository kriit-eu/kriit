...
// ----------------------------------------------------------
// Robust Podman / Docker detection
// Try to *run* podman-compose – if it works, use Podman.
// ----------------------------------------------------------
let podman = false;
try {
    await $`podman-compose --help`.quiet();   // exits 0 quickly
    podman = true;
} catch {
    // not a Podman environment – assume Docker
    podman = false;
}
...
if (
    ...
    try {
        if (podman) {
            // podman-compose sometimes exits 1 here – ignore it
            await $`podman-compose --version`.quiet();
        } else {
            await $`${engine} compose version`.quiet();
        }
    } catch (_) {
        /* ignore non-zero */
    }
    process.exit(0);          // ← always succeed for the wrapper
}
...
// ------------------------------------------------------------------
// Special-case “--version” so we don’t add any -f arguments
// podman-compose exits non-zero if -f is passed with --version.
// ------------------------------------------------------------------
if (args.length === 1 && (args[0] === '--version' || args[0] === 'version')) {
    if (podman) {
        await $`podman-compose --version`;
    } else {
        await $`${engine} compose version`;
    }
    process.exit(0);
}
