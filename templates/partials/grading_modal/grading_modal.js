...
function openGradingModal(row) {
  ...
  if (criteriaContainer) {
    criteriaContainer.innerHTML =
      '<p class="text-muted">Laen kriteeriumeid...</p>';
  }
...
function loadAssignmentDataAjax(assignmentId, studentId) {
  fetch(
    `${
      window.BASE_URL || "/"
    }assignments/ajax_getAssignmentDetails?assignmentId=${assignmentId}&studentId=${studentId}`
  )
    ...
    .then((data) => {
      if (data.status === 200) {
        ...
        // Update solution URL
        const solutionUrlDetails =
          document.getElementById("solutionUrlDetails");
        ...
        if (openApiButton) {
          openApiButton.style.display = assignmentData.assignmentInvolvesOpenApi
            ? "inline-block"
            : "none";
        }
        ...
        if (instructionsDiv) {
          instructionsDiv.innerHTML =
            '<p class="text-danger">Viga andmete laadimisel</p>';
        } else {
          console.error("Instructions div not found in error handler");
        }
        if (criteriaContainer) {
          criteriaContainer.innerHTML =
            '<p class="text-danger">Viga kriteeriumide laadimisel</p>';
        }
      }
    })
    .catch((error) => {
...
function loadCriteria(criteriaData) {
  ...
  try {
    ...
    const criteriaCheckboxes = criteriaContainer.querySelectorAll(
      'input[type="checkbox"]'
    );
...
function saveGradeAndComment() {
  ...
  const criteriaCheckboxes = document.querySelectorAll(
    '#criteriaContainer input[type="checkbox"]'
  );
  criteriaCheckboxes.forEach((checkbox) => {
    ...
    .then((data) => {
      if (data.status === 200) {
        ...
        if (data.data.gradedAt) {
          updateTableRowTimestamps(
            currentAssignmentId,
            currentUserId,
            data.data.gradedAt,
            data.data.daysDifference
          );
        }

        // Add grade badge to table row
        addGradeBadgeToTableRow(
          currentAssignmentId,
          currentUserId,
          selectedGrade
        );
  ...
  fetch(`${window.BASE_URL || "/"}grading/getMessages`, {
    ...
    .then((data) => {
      if (data.status === 200) {
        ...
        messagesContainer.innerHTML = `<p class="text-danger">Viga sõnumite laadimisel: ${
          data.message || "Tundmatu viga"
        }</p>`;
      }
    })
    .catch((error) => {
...
function displayMessages(messages) {
  ...
  messages.forEach((message) => {
    if (!message.isNotification) {
      ...
      let imageHtml = "";
      if (message.imageId) {
        imageHtml = `<div class="mt-2">${displayMessageImage(
          message.imageId
        )}</div>`;
      }
      ...
      const isOwnMessage =
        currentUserId && message.userId && message.userId == currentUserId;
                        ...
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between mb-2">
                                <span class="message-author">${
                                  message.userName || "Tundmatu"
                                }</span>
                                <span class="message-time">${messageDate}</span>
                            </div>
                            <div class="message-content">${parseMarkdown(
                              message.content
                            )}</div>
                            ${imageHtml}
                        </div>
  ...
  messagesContainer.innerHTML =
    messagesHtml || '<p class="text-muted">Sõnumeid pole veel</p>';

  // Scroll to bottom only if the container is scrollable (i.e., has overflow)
  try {
    if (messagesContainer.scrollHeight > messagesContainer.clientHeight) {
      messagesContainer.scrollTop = messagesContainer.scrollHeight;
    }
  } catch (e) {
    // If element not found or any error, silently ignore
  }
}
...
function updateTableRowGrade(assignmentId, userId, grade) {
  ...
  const rows = document.querySelectorAll(
    ".grading-row, tr[data-assignment-id]"
  );
  rows.forEach((row) => {
    if (
 ...
 */
function updateTableRowTimestamps(
  assignmentId,
  userId,
  gradedAt,
  daysDifference
  ...
  try {
    ...
    const targetRow = document.querySelector(
      `tr[data-assignment-id="${assignmentId}"][data-user-id="${userId}"], .grading-row[data-assignment-id="${assignmentId}"][data-user-id="${userId}"]`
    );

    if (!targetRow) {
      console.warn("Table row not found for updating timestamps:", {
        assignmentId,
        userId,
      });
      return;
    ...
    if (gradedCell && gradedAt) {
      ...
      // Update tooltip
      const fullDate =
        gradedDate.toLocaleDateString("et-EE", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        }) +
    ...
    if (differenceCell) {
      ...
      if (daysDifference) {
        differenceCell.setAttribute(
          "title",
          `Hindamine võttis ${daysDifference} päeva`
        );
      } else {
...
function addGradeBadgeToTableRow(assignmentId, userId, grade) {
  try {
    ...
    const gradeCell = document.querySelector(
      `td[data-assignment-id="${assignmentId}"][data-student-id="${userId}"]`
    );
    ...
    if (!targetRow) {
      console.warn("Table row not found for adding grade badge:", {
        assignmentId,
        userId,
      });
      return;
    ...
    const fallbackGradeCell = targetRow.querySelector(
      "td[data-student-id], td[data-grade], td.grade-cell, td:last-child"
    );
...
function initializeCommentToggle() {
  ...
  toggleBtn.addEventListener("click", function (e) {
    ...
    const isExpanded = commentSection.classList.contains(
      ...
      invalidFiles.forEach(({ file, errors }) => {
        ...
        errorDiv.innerHTML = `
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="file-info">${formatFileSize(
                          file.size
                        )}</span>
                    </div>
                    <div class="text-danger small mt-1">${errors.join(
                      ", "
                    )}</div>
                `;
