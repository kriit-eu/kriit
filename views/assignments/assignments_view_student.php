...
foreach ($assignment['students'] as $s): 
    ...
    if ($s['studentId'] == $currentUserId): 
        ...
        if ($isPositiveGrade) {
            ...
            // Mark current user's cell yellow when assignment status is Esitamata (1)
            // or Kontrollimisel (2) so the student sees their own waiting/not-submitted state
            $assignmentStatusId = $s['assignmentStatusId'] ?? null;
            if ($assignmentStatusId === 1 || $assignmentStatusId === 2) {
            ...
            <div id="assignments-container-right" style="overflow-x: auto; flex: 1;">
                ...
                <div class="assignments-body">
                    ...
                    foreach ($assignment['students'] as $s):
                        ...
                        // Also show yellow for 'Kontrollimisel' (waiting for review, assignmentStatusId = 2)
                        if ($assignmentStatusId === 1 || $assignmentStatusId === 2) {
                            $showYellow = true;
                        }
